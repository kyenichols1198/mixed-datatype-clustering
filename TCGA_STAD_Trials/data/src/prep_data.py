...
def prep_multi_omics(input_dir, output_name, output_path, labels_path, label_col_name, datatype_tag_dict, encoding, overwrite=False):
    if overwrite == True:
        ...
        mrna = rem_low_var(mrna.round(4), var_thresh=1.00)
        mrna = format_barcodes(mrna)
        print(mrna.shape)
        ...
        to_save = pickle.load(open(os.path.join(output_path, '%s.pickle' % output_name), "rb"))
        labels = to_save["labels"]
        print(labels.head())
